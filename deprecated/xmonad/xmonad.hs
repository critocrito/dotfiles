...
myTerminal           = "alacritty -e zsh -i"   -- which terminal software to use
...
myKeyBindings =
  [
    ...
    , ((myModMask .|. shiftMask, xK_l), spawn "xset s activate")
...
--   , (className =? "urxvt") --> doF (W.shift "2:Term")
...
myManageHooks = composeAll . concat $
    ...
    , [(className =? x <||> title =? x <||> resource =? x) --> doShiftAndGo "9:SQL" | x <- mySqlShifts]
    , [(className =? x <||> title =? x <||> resource =? x) --> doShiftAndGo "0:VM" | x <- myVmShifts]
    ]
    where
    ...
    myIgnores = ["synapse", "stalonetray", "gimp"]
    -- mySinks = ["gimp"]
    mySinks = []
    -- Run VLC, firefox in fullscreen
    myFullscreens = []
    ...
    myHubShifts    = ["chromium-browser", "Chromium-browser", "pavucontrol",
                      "Pavucontrol", "calibre-gui", "Calibre-gui"]
    myMusicShifts  = ["Clementine"]
    myDbgShifts    = []
    mySqlShifts    = ["mysql-workbench-bin", "Mysql-workbench-bin", "pgadmin3",
                      "Pgadmin3"]
    myVmShifts     = ["virt-manager", "Virt-manager"]
